...
public class WorldGenAzaleaTree extends WorldGenAbstractTree {
    ...
    private static final IBlockState DEFAULT_LEAF = ModBlocks.AZALEA_LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false);
    private static final IBlockState FLOWER_LEAF = ModBlocks.FLOWERING_AZALEA_LEAVES.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false);
            ...
            else
            {
                ...
                if (state.getBlock().canSustainPlant(state, worldIn, position.down(), net.minecraft.util.EnumFacing.UP, (net.minecraft.block.BlockSapling)Blocks.SAPLING) && position.getY() < worldIn.getHeight() - i - 1)
                {
                            ...
                            for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2)
                            {
                                ...
                                if (Math.abs(l1) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0)
                                {
                                    ...
                                    if (state.getBlock().isAir(state, worldIn, blockpos) || state.getBlock().isLeaves(state, worldIn, blockpos) || state.getMaterial() == Material.VINE)
                                    {
                                        ...
                                        {
                                            this.setBlockAndNotifyAdequately(worldIn, blockpos, FLOWER_LEAF);
                                        }else {
                    ...
                    for (int j3 = 0; j3 < i; ++j3)
                    {
                        ...
                        {
                            this.setBlockAndNotifyAdequately(worldIn, position.up(j3), DEFAULT_TRUNK);
                        }
                    }
                    return true;
                }
